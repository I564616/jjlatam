...
/**
 * Generated class for type <code>Jnjb2bglobalCoreManager</code>.
 */
@SuppressWarnings({"deprecation","unused","cast"})
public abstract class GeneratedJnjb2bglobalCoreManager extends Extension
{
	...
	public HeaderNavigationComponent createHeaderNavigationComponent(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( Jnjb2bglobalCoreConstants.TC.HEADERNAVIGATIONCOMPONENT );
	...
	public MultipleCatalogsSyncCronJob createMultipleCatalogsSyncCronJob(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( Jnjb2bglobalCoreConstants.TC.MULTIPLECATALOGSSYNCCRONJOB );
	...
	public OrganizationOrdersReportingCronJob createOrganizationOrdersReportingCronJob(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( Jnjb2bglobalCoreConstants.TC.ORGANIZATIONORDERSREPORTINGCRONJOB );
	...
	public OrganizationOrderStatistics createOrganizationOrderStatistics(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( Jnjb2bglobalCoreConstants.TC.ORGANIZATIONORDERSTATISTICS );
	...
	public PowertoolsSizeVariantProduct createPowertoolsSizeVariantProduct(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( Jnjb2bglobalCoreConstants.TC.POWERTOOLSSIZEVARIANTPRODUCT );
	...
	{
		return Jnjb2bglobalCoreConstants.EXTENSIONNAME;
	}
